...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while cur_place.entrance is not beehive:
            if cur_place.bees and self.min_range <= dist <= self.max_range:
                break
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        ...
        if not self.have_true_queen:
            self.is_true_queen = True
            QueenAnt.have_true_queen = True
        else:
    ...
    def action(self, gamestate):
        ...
        if self.is_true_queen:
            ...
            while cur_place.exit is not None:
                ...
                if isinstance(ant,
                              ContainerAnt) and ant.contained_ant is not None and ant.contained_ant not in self.buffed_ants:
                    ant.contained_ant.damage *= 2
                    self.buffed_ants.append(ant.contained_ant)
        else:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if self.direction == -1:
            if type(self.place.entrance) != Hive:
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and target not in self.scary_bees:
            self.scary_bees.append(target)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while cur_place is not beehive:
            if isinstance(cur_place.ant, ContainerAnt):
                insects[cur_place.ant] = distance
                if cur_place.ant.contained_ant is not self and cur_place.ant.contained_ant is not None:
                    insects[cur_place.ant.contained_ant] = distance
            else:
                if cur_place.ant is not self and cur_place.ant is not None:
                    insects[cur_place.ant] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        return max(0, self.damage - self.insects_shot*0.05 - distance*0.2)
        # END Problem OPTIONAL
