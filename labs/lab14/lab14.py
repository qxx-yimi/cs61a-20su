...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            rest_result, rest_cost = helper_align(skeleton_idx+1, code_idx+1)
            all_result = skel_char + rest_result
            possibilities.append((all_result, rest_cost))
        # Insert
        rest_result, rest_cost = helper_align(skeleton_idx, code_idx+1)
        all_result, all_cost = '+[' + code_char + ']' + rest_result, 1 + rest_cost
        possibilities.append((all_result, all_cost))
        # Delete
        rest_result, rest_cost = helper_align(skeleton_idx+1, code_idx)
        all_result, all_cost = '-[' + skel_char + ']' + rest_result, 1 + rest_cost
        possibilities.append((all_result, all_cost))
...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    def num_splits_help(index, sums):
        if index >= len(s):
            return abs(sum(s)-sums-sums) <= d
        return num_splits_help(index + 1, sums + s[index]) + num_splits_help(index + 1, sums)

    return num_splits_help(0, 0) // 2
...
def insert(link, value, index):
    ...
    if index == 0 and link is not Link.empty:
        node = Link(link.first)
        link.first = value
        link.rest, node.rest = node, link.rest
