...
def balanced(m):
    ...
    return balanced(end(left(m))) and balanced(end(right(m))) \
        and length(left(m)) * total_weight(end(left(m))) == length(right(m)) * total_weight(end(right(m)))
...
def has_path(t, phrase):
    ...
    bran_has = False
    for b in branches(t):
        bran_has = bran_has or has_path(b, phrase[1:])
    return label(t) == phrase[0] and bran_has
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) > 0 or upper_bound(y) < 0, 'this interval spans zero'
    reciprocal_y = interval(1 / upper_bound(y), 1 / lower_bound(y))
    return mul_interval(x, reciprocal_y)


def multiple_references_explanation():
    return """par2涉及到的r1与r2之间的运算次数比par1涉及到的更少"""
...
def quadratic(x, a, b, c):
    ...
    f1, f2, f3 = f(lower_bound(x)), f(-b / (2 * a)), f(upper_bound(x))
    if lower_bound(x) < -b / (2 * a) < upper_bound(x):
        return interval(min(f1, f2, f3), max(f1, f2, f3))
    return interval(min(f1, f3), max(f1, f3))
