...
def is_bst(t):
    ...
    def bst_min(tree):
        if tree.is_leaf():
            return tree.label
        return min(tree.label, min([bst_min(b) for b in tree.branches]))

    def bst_max(tree):
        if tree.is_leaf():
            return tree.label
        return max(tree.label, max([bst_max(b) for b in tree.branches]))

    if t.is_leaf():
        return True
    bs = [b for b in t.branches]
    if len(bs) == 1:
        return is_bst(bs[0]) and (t.label < bst_min(bs[0]) or t.label >= bst_max(bs[0]))
    return is_bst(bs[0]) and is_bst(bs[1]) and bst_min(bs[1]) > t.label >= bst_max(bs[0])
...
def path_yielder(t, value):
    ...
    # def helper(tree, path, v):
    #     path.append(tree.label)
    #     if tree.label == v:
    #         yield path
    #     for b in tree.branches:
    #         yield from helper(b, path.copy(), v)
    #
...
def deep_map(f, link):
    ...
    """
    "*** YOUR CODE HERE ***"
    if type(link.first) != Link and link.rest is Link.empty:
        return Link(f(link.first))
    elif type(link.first) != Link:
        rest = deep_map(f, link.rest)
        return Link(f(link.first), rest)
