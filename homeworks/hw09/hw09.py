...
def interleaved_sum(n, odd_term, even_term):
    ...
    def interleaved_sum_help(i):
        if i + 1 <= n:
            return odd_term(i) + even_term(i + 1) + interleaved_sum_help(i + 2)
        elif i <= n:
            return odd_term(i)

    return interleaved_sum_help(1)
...
def mutate_reverse(link):
    ...
    while head is not Link.empty:
        values.append(head.first)
        head = head.rest
    head = link
    while head is not Link.empty:
        head.first = values.pop()
        head = head.rest
