...
def g_iter(n):
    ...
    i = 0
    while i < n - 3:
        f1, f2, cur = f2, cur, cur + 2 * f2 + 3 * f1
        i += 1
    return cur
...
def count_change(total):
    ...
    def count_help(sum_count, n):
        if sum_count == 0:
            return 1
        if sum_count < 0 or sum_count < n:
            return 0
        return count_help(sum_count - n, n) + count_help(sum_count, n * 2)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda f: (lambda x: f(f, x)))(lambda gg, n: 1 if n == 0 else mul(n, gg(gg, sub(n, 1))))
